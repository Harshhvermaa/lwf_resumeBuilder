...
export default function SidebarPortraitTemplate({ data }: TemplateProps) {
  ...
  const role = data.experience?.[0]?.title || 'Professional';
      ...
      <div className="grid grid-cols-[260px_1fr] min-h-[1123px]">
        <aside className="bg-slate-900 text-white px-6 py-8">
          <div className="flex flex-col items-center">
            <div className="h-28 w-28 overflow-hidden rounded-full border-4 border-slate-200 bg-slate-700">
          ...
          <div className="mt-8 space-y-6">
            <div>
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-200">
                ...
                {data.phone && (
                  <div className="flex items-start gap-2">
                    <Phone className="h-4 w-4 mt-0.5 text-slate-300" />
                    <span>{data.phone}</span>
                  </div>
                )}
                {data.email && (
                  <div className="flex items-start gap-2">
                    <Mail className="h-4 w-4 mt-0.5 text-slate-300" />
            ...
            {skills.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-300">Skills</h3>
            ...
            {certifications.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-300">Languages</h3>
                <ul className="mt-4 space-y-2 text-sm text-slate-100">
            ...
            {hobbies.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-300">Hobbies</h3>
                <ul className="mt-4 space-y-2 text-sm text-slate-100">
        ...
        <section className="px-10 py-10">
          <div className="border-b border-slate-200 pb-5">
            ...
            <p className="text-sm uppercase tracking-[0.22em] text-slate-500 mt-1">{role}</p>
          </div>
                      ...
                      <ul className="mt-3 space-y-1 text-xs text-slate-600">
                        {getDescriptionPoints(exp.description, 3).map((point, idx) => (
                          <li key={idx} className="flex items-start gap-2">
                            <span className="mt-2 h-1 w-1 rounded-full bg-slate-900" />
                            <span>{point}</span>
                          </li>
                        ))}
                      </ul>
